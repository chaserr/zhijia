...
@interface KMDatePicker () {
    ...
    NSInteger _yearIndex1;
    NSInteger _monthIndex1;
    NSInteger _dayIndex;
...
//
    ...
    if (!_maxLimitedDate) {
        _maxLimitedDate = [DateHelper dateFromString:currentyearString withFormat:nil];
    }
    ...
    _yearIndex1 =[_datePickerDateScrollTo.year1 integerValue] - [_datePickerDateMinLimited.year1 integerValue];
    // 月
    ...
    _monthIndex1 = [_datePickerDateScrollTo.month1 integerValue] - 1;
...
- (void)scrollToDateIndexPosition {
    ...
    switch (_datePickerStyle) {
        ...
        case KMDatePickerStyleYearMonthYearMonth:{
        
            arrIndex = @[
                         [NSNumber numberWithInteger:_yearIndex],
                         [NSNumber numberWithInteger:_monthIndex],
                         [NSNumber numberWithInteger:_yearIndex1],
                         [NSNumber numberWithInteger:_monthIndex1],
                         ];
            break;
        }
    }
...
- (void)playDelegateAfterSelectedRow {
    NSString *dateStr = [NSString stringWithFormat:@"%@.%@ ~ %@.%@",
                         _mArrYear[_yearIndex],
                         _mArrMonth[_monthIndex],
                         _mArrYear[_yearIndex1],
                         _mArrMonth[_monthIndex1]
                         //                         _mArrDay[_dayIndex],
                         //                         _mArrHour[_hourIndex],
                         //                         _mArrMinute[_minuteIndex]
                         ];
    
    if ( [_mArrYear[_yearIndex1] intValue] >= [_datePickerDateScrollTo.year1 intValue] &&[_mArrMonth[_monthIndex1] intValue] >= [_datePickerDateScrollTo.month1 intValue]) {
        _monthIndex1 = [_datePickerDateScrollTo.month1 intValue] - 1;
        [self scrollToDateIndexPosition];
        dateStr = [NSString stringWithFormat:@"%@.%@ ~ 至今",
                   _mArrYear[_yearIndex],
                   _mArrMonth[_monthIndex]
                   ];
...
- (CGFloat)pickerView:(UIPickerView *)pickerView widthForComponent:(NSInteger)component {
    ...
    switch (_datePickerStyle) {
        ...
        case KMDatePickerStyleYearMonthYearMonth:{
            ...
            widthOfAverage = (kWidthOfTotal - 20.0 - 15.0) / 4;
            switch (component) {
                case 0:
                    width = widthOfAverage + 20.0;
                    [self addUnitLabel:@"年" withPointX:width/2 + 24.0];
                ...
                case 1:
                    width = widthOfAverage;
                    [self addUnitLabel:@"到" withPointX:(kWidthOfTotal -  20 )/2];
                    break;
                case 2:
                    width = widthOfAverage + 20;
                    [self addUnitLabel:@"年" withPointX:(2*widthOfAverage + 20.0) + width/2 + 30.0];
                    break;
                case 3:
                    width = widthOfAverage;
                    //                    [self addUnitLabel:@"月" withPointX:(3*widthOfAverage + 20.0) + width/2 + 28.0];
                    break;
            }
            break;
        }
...
//    _scrollToDate = [DateHelper dateFromString:dateStr withFormat:nil];
    NSDate *date = [NSDate date];
    _datePickerDateScrollTo = [[KMDatePickerDateModel alloc] initWithDate:date];
    if ( [_mArrYear[_yearIndex1] intValue] >= [_datePickerDateScrollTo.year1 intValue] &&[_mArrMonth[_monthIndex1] intValue] >= [_datePickerDateScrollTo.month1 intValue]) {
        _monthIndex1 = [_datePickerDateScrollTo.month1 intValue] - 1;
        [self scrollToDateIndexPosition];
        dateStr = [NSString stringWithFormat:@"%@.%@ ~ 至今",
    ...
    if (_yearIndex>=_yearIndex1) {
        
        _yearIndex = _yearIndex1;
        if (_monthIndex >= _monthIndex1) {
            _monthIndex = _monthIndex1;
        }
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(datePicker:didSelectDate:)]) {
            
            [_delegate datePicker:self didSelectDate:dateStr];
        }
    }
