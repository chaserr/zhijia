...
- (void)viewDidLoad
{
    ...
    if (CGRectEqualToRect(_previewRect, CGRectZero)) {
        self.previewRect = CGRectMake(0, 0, SC_APP_SIZE.width, SCREEN_HEIGHT - CGRectGetHeight(_bottomContainerView.frame));
    }
...
//        [self addTopViewWithText:@"拍照"];
    ...
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        [GBHUDVIEW showTips:@"设备不支持拍照功能，给个妹纸给你喵喵T_T" autoHideTime:1];

        UIImageView *imgView = [[UIImageView alloc] initWithFrame:CGRectMake(0, CAMERA_TOPVIEW_HEIGHT, self.view.frame.size.width, SCREEN_HEIGHT - CGRectGetHeight(_bottomContainerView.frame) - 44)];
...
- (void)addbottomContainerView {
    ...
    CGFloat bottomY = SCREEN_HEIGHT - 140;
...
- (void)addCameraMenuView {
    ...
    //拍照按钮  
    CGFloat cameraBtnLength = 65;
    
    LeafButton *btn = [[LeafButton alloc] initWithFrame:CGRectMake((SCREEN_WIDTH - cameraBtnLength) / 2, (_bottomContainerView.frame.size.height - cameraBtnLength - 20) , cameraBtnLength, cameraBtnLength)];
    btn.type = LeafButtonTypeCamera;
    [btn setClickedBlock:^(LeafButton *button) {
        
        [self takePictureBtnPressed];
        
    }];
    [_bottomContainerView addSubview:btn];
    
    CGPoint cancleCenter = CGPointMake(20, CGRectGetMidY(btn.frame) - 20);
    self.cancleBtn = [UIButton buttonWithType:(UIButtonTypeCustom)];
    _cancleBtn.frame = CGRectMake(20, cancleCenter.y, 40, 40);
    [_cancleBtn setTitle:@"取消" forState:(UIControlStateNormal)];
    [_bottomContainerView addSubview:_cancleBtn];
...
- (void)addMenuViewButtons {
    NSMutableArray *normalArr = [[NSMutableArray alloc] initWithObjects:@"icon_btn_camera_flash_auto",@"btn_video_flip_camera", nil];
    NSMutableArray *actionArr = [[NSMutableArray alloc] initWithObjects:@"flashBtnPressed:", @"switchCameraBtnPressed:", nil];
    CGFloat eachW = (SC_APP_SIZE.width - 20) / actionArr.count;
    
        for (int i = 0; i < actionArr.count; i++) {
            
            CGFloat theH = CAMERA_MENU_VIEW_HEIGH;
        
        UIButton * btn = [self buildButton:CGRectMake((eachW + 10)* i, 0, eachW, theH)
...
- (UIButton*)buildButton:(CGRect)frame
    ...
    btn.frame = frame;
    if ([normalImgStr isEqualToString:@"icon_btn_camera_flash_auto"]) {
...
//对焦的框
- (void)addFocusView {
    UIImageView *imgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"touch_focus"]];
    imgView.alpha = 0;
    [self.view addSubview:imgView];
...
- (void)showCameraCover:(BOOL)toShow {
    ...
    [UIView animateWithDuration:0.38f animations:^{
        ...
        upFrame.size.height = (toShow ? _previewRect.size.height / 2 : 0);
        _doneCameraUpView.frame = upFrame;
        
        CGRect downFrame = _doneCameraDownView.frame;
        downFrame.origin.y = (toShow ? _previewRect.size.height / 2 : _bottomContainerView.frame.origin.y);
        downFrame.size.height = (toShow ? _previewRect.size.height / 2 : 0);
...
//拍照页面，拍照按钮
- (void)takePictureBtnPressed{
#if SWITCH_SHOW_DEFAULT_IMAGE_FOR_NONE_CAMERA
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        [GBHUDVIEW showTips:@"设备不支持拍照功能T_T" autoHideTime:1];
        return;
    }
#endif
