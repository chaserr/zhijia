...
//                                      };
    ...
    _controllerScrollView.frame = CGRectMake(0.0, 0, self.view.frame.size.width, self.view.frame.size.height);
//    _controllerScrollView.userInteractionEnabled = NO;
    _controllerScrollView.backgroundColor = [UIColor colorWithRed:1.000 green:0.000 blue:0.009 alpha:0.076];
    _controllerScrollView.alpha = 0;
    [self.view addSubview:_controllerScrollView];

    
    // Set up menu scroll view
    _menuScrollView.translatesAutoresizingMaskIntoConstraints = NO;
    _menuScrollView.frame = CGRectMake(0.0, self.view.frame.size.height - _menuHeight, self.view.frame.size.width, _menuHeight);
...
//        }
        ...
        if (_useMenuLikeSegmentedControl) {
            menuItemFrame = CGRectMake(self.view.frame.size.width / (CGFloat)_controllerArray.count * (CGFloat)index, self.view.frame.size.height - _menuHeight, (CGFloat)self.view.frame.size.width / (CGFloat)_controllerArray.count, _menuHeight);
            ...
            menuItemFrame = CGRectMake(_totalMenuItemWidthIfDifferentWidths + _menuMargin + (_menuMargin * index), self.view.frame.size.height - _menuHeight, _menuItemWidth, _menuHeight);
        ...
        if ([scrollView isEqual:_controllerScrollView]) {
            if (scrollView.contentOffset.x >= 0.0 && scrollView.contentOffset.x <= ((CGFloat)(_controllerArray.count - 1) * self.view.frame.size.width)) {
                ...
                if ((_currentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation)) || (!_currentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation))){
                    ...
                    float offset =  scrollView.contentOffset.x - (_currentPageIndex * scrollView.frame.size.width);
                    float OldMin = 414*_currentPageIndex;
                    int Value = scrollView.contentOffset.x -OldMin;
                    float alpha = (Value% 414) / 414.f;
                    //                    CGFloat alpha= 1 -((scrollView.frame.size.width -scrollView.contentOffset.x)/ (scrollView.frame.size.width));
                    if (offset < 0) {
                        if (_currentPageIndex == 0) {
                            UIViewController *currentVC = _controllerArray[_currentPageIndex];
                            currentVC.view.alpha = 1;
                            
                        }else{
                            
                            UIViewController *currentVC = _controllerArray[_currentPageIndex];
                            currentVC.view.alpha = alpha;
                        }
                        ...
                        if(_currentPageIndex == _controllerArray.count-1) {
                            ...
                            currentVC.view.alpha = 1.0 - alpha;
                        } else {
                            
                            UIViewController *nextVC = _controllerArray[_currentPageIndex + 1];
                            nextVC.view.alpha = alpha;
                            UIViewController *currentVC = _controllerArray[_currentPageIndex];
                            currentVC.view.alpha = 1 - nextVC.view.alpha;
                        }
                    }
                    else{
                        [_controllerArray enumerateObjectsUsingBlock:^(UIViewController *viewController, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)addPageAtIndex:(NSInteger)index
{
    ...
    [self.view bringSubviewToFront:_controllerScrollView];
...
- (void)viewDidLayoutSubviews
{
    ...
    // Configure controller scroll view content size
    _controllerScrollView.contentSize = CGSizeMake(self.view.frame.size.width * (CGFloat)_controllerArray.count, self.view.frame.size.height);
    _menuScrollView.frame = CGRectMake(0, self.view.frame.size.height - 140 + 10, self.view.frame.size.width, _menuHeight);
    _controllerScrollView.frame = CGRectMake(0.0, 0, self.view.frame.size.width, self.view.frame.size.height);
    ...
    if ((oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsLandscape(orientation)) || (!oldCurrentOrientationIsPortrait && UIInterfaceOrientationIsPortrait(orientation))){
        ...
        for (UIView *view in _controllerScrollView.subviews) {
            view.frame = CGRectMake(self.view.frame.size.width * (CGFloat)(_currentPageIndex), _menuHeight, _controllerScrollView.frame.size.width, self.view.frame.size.height);
        }
