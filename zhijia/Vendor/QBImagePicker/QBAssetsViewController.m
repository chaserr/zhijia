...
@interface QBAssetsViewController () <PHPhotoLibraryChangeObserver, UICollectionViewDelegateFlowLayout,UICollectionViewDataSource, UICollectionViewDelegate>
...
{
    [super viewDidLoad];
    self.navigationTitleLabel.text= @"选择图片";
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(updateSelect:) name:kNotificationBackSmallIcon object:nil];
}

- (void)updateSelect:(NSNotification *) notification
{
    NSArray * selectArray = notification.object;
    [self.imagePickerController.selectedAssets removeAllObjects];
    [self.imagePickerController.selectedAssets addObjectsFromArray:selectArray];
...
- (UICollectionView *)collectionView{
    ...
    if (nil == _collectionView) {
        CGFloat imageWidth = (CGRectGetWidth(rect) - 6)/4;
        UICollectionViewFlowLayout * flowLayout = [[UICollectionViewFlowLayout alloc] init];
        [flowLayout setItemSize:CGSizeMake(imageWidth, imageWidth)];
...
//    CGRect rect = self.view.bounds;
    ...
    [_collectionView registerClass:[QBAssetCell class] forCellWithReuseIdentifier:CELL_IDENTIFIER];
...
- (void)updateCachedAssets
{
    ...
    if (delta > CGRectGetHeight(self.collectionView.bounds) / 3.0) {
        ...
        CGSize itemSize = [(UICollectionViewFlowLayout *)_collectionView.collectionViewLayout itemSize];
...
#pragma mark YYImageViewControllerDelegate
...
#pragma mark QBAssetCellDelegate

- (void)imageTap:(QBAssetCell *)assetCell
{
    GBLookBigImagCollectionVC *viewController = [[GBLookBigImagCollectionVC alloc] initWithImageArray:self.fetchResult atIndex:assetCell.indexPath.row withSelectedImage:self.imagePickerController.selectedAssets.array withType:YYScanImageComeReleaseDynimic];
     viewController.delegate = self;
     viewController.maxSelectCount = self.imagePickerController.maximumNumberOfSelection;
...
- (void) imageSelectedChangeWith:(QBAssetCell *) assetCell
{
    ...
    NSIndexPath * indexPath = assetCell.indexPath;
    PHAsset *asset = self.fetchResult[indexPath.item];
 
    if (imagePickerController.allowsMultipleSelection ) {
        if ([selectedAssets containsObject:asset]) {
            [selectedAssets removeObject:asset];
            [assetCell  setIsSelected:NO];
        }
        else{
            if (selectedAssets.count < imagePickerController.maximumNumberOfSelection) {
                [selectedAssets addObject:asset];
                [assetCell setIsSelected:YES];
            }else {
                
                [GBHUDVIEW showTips:[NSString stringWithFormat:@"最多选%ld张图片", imagePickerController.maximumNumberOfSelection]];
            }
        }
    }
    self.doneButton.enabled = [self isMinimumSelectionLimitFulfilled];
...
{
    QBAssetCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:CELL_IDENTIFIER forIndexPath:indexPath];
...
- (BOOL)collectionView:(UICollectionView *)collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self isMaximumSelectionLimitReached]) {
        ...
        [GBHUDVIEW showTips:[NSString stringWithFormat:@"最多选%ld张图片", _imagePickerController.maximumNumberOfSelection]];
    }
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
        ...
        GBLookBigImagCollectionVC *viewController = [[GBLookBigImagCollectionVC alloc] initWithImageArray:self.fetchResult atIndex:indexPath.row withSelectedImage:self.imagePickerController.selectedAssets.array withType:YYScanImageComeReleaseDynimic];
        //    YYImageViewController * viewController = [[YYImageViewController alloc] initWithImageArray:self.fetchResult atIndex:indexPath.row withSelectedImage:self.imagePickerController.selectedAssets.array withType:YYScanImageComeReleaseDynimic];
        viewController.delegate = self;
        viewController.maxSelectCount = self.imagePickerController.maximumNumberOfSelection;
        [self.navigationController pushViewController:viewController animated:YES];
