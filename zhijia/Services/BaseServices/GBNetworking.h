 ...
 */
typedef void (^GBDownloadProgress)(int64_t bytesRead,
                                    int64_t totalBytesRead);

typedef GBDownloadProgress GBGetProgress;
typedef GBDownloadProgress GBPostProgress;
 ...
 */
typedef void (^GBUploadProgress)(int64_t bytesWritten,
                                  int64_t totalBytesWritten);

typedef NS_ENUM(NSUInteger, GBResponseType) {
    kGBResponseTypeJSON = 1, // 默认
    kGBResponseTypeXML  = 2, // XML
    // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
    kGBResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, GBRequestType) {
    kGBRequestTypeJSON = 1, // 默认
    kGBRequestTypePlainText  = 2 // 普通text/html
};
...
// 且处理，请转换成对应的子类类型
typedef NSURLSessionTask GBURLSessionTask;
 ...
 */
typedef void(^GBResponseSuccess)(NSURLSessionDataTask * _Nullable task, id  _Nullable responseDict);
 ...
 *  @param error 错误信息
 */
typedef void(^GBResponseFail)(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error);
